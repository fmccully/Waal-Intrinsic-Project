...
## @knitr setup2
#setup
setwd("~/R Practice/Wandering albatross/Model R Markdown")
library(dplyr)
library(MuMIn)
library(ggplot2)
library(lme4)
...
total_df2<-read.csv("waal_dataset.csv")
total_df2$X<-NULL
...
with(incu_new, tapply(tag, age, FUN = function(x) length(unique(x)))) # number birds per age group (individual birds may appear more than once)
table(incu_new$cycle) # no obs per year

## @knitr cent-incu
# measures of centrality- incubation
incu_new%>%summarise(average=mean(no_hours),med=median(no_hours), sd=sd(no_hours))
# males and female comparison
incu_new%>%group_by(sex)%>%summarise(average=mean(no_hours),med=median(no_hours), sd=sd(no_hours))
# loggered and partner bird (unloggered) comparison
incu_new%>%group_by(partner_type)%>%summarise(average=mean(no_hours),med= median(no_hours), sd=sd(no_hours))
# interannual comparison
incu_new%>%group_by(cycle)%>%summarise(average=mean(no_hours),med=median(no_hours), sd=sd(no_hours))

## @knitr des-brooding
#sample sizes- brooding
unique(brood_new$cycle_pair)
...
## @knitr sam-sp-br
with(brood_new, tapply(pair_id, cycle, FUN = function(x) length(unique(x)))) # pair sample size per year
with(brood_new, tapply(tag, age, FUN = function(x) length(unique(x)))) # number birds per age group 
table(brood_new$cycle) # no obs per year

## @knitr cent-brood
# measures of centrality- brooding
brood_new%>%summarise(average=mean(no_hours),med=median(no_hours), sd=sd(no_hours))
#males and females comparison
brood_new%>%group_by(sex)%>%summarise(average=mean(no_hours),med=median(no_hours), sd=sd(no_hours))
# loggered and partner birds comparison
brood_new%>%group_by(partner_type)%>%summarise(average=mean(no_hours),med= median(no_hours), sd=sd(no_hours))
#interannual comparison 
brood_new%>%group_by(cycle)%>%summarise(average=mean(no_hours),med=median(no_hours), sd=sd(no_hours))
...
## @knitr incu-resc
#rescaling
incu_new2$age_sc<-scale(incu_new2$age)
incu_new2$p_age_sc<-scale(incu_new2$p_age)
incu_new2$age_sq_sc<-scale(incu_new2$age_sq)
incu_new2$p_age_sq_sc<-scale(incu_new2$p_age_sq)
incu_new2$pers_sc<-scale(incu_new2$pers)
incu_new2$num_days_sc<-scale(incu_new2$NumDays)
incu_new2$p_pers_sc<-scale(incu_new2$p_pers)

## @knitr incu-pers-cat
# adding boldness bold/shy category variable incubation
# FOR GRAPHICAL REPRESENTATION NOT USED IN ANALYSIS
for (a in 1:(nrow(incu_new2))) {
  if (incu_new2$pers_sc[a] < 0)
  {incu_new2$pers_type[a]<- "shy"}
  else {incu_new2$pers_type[a] <-"bold"}
}

for (a in 1:(nrow(incu_new2))) {
  if (incu_new2$p_pers_sc[a] < 0)
  {incu_new2$p_pers_type[a]<- "shy"}
  else {incu_new2$p_pers_type[a] <-"bold"}
}

incu_new2$pers_type<-as.factor(incu_new2$pers_type)
incu_new2$p_pers_type<-as.factor(incu_new2$p_pers_type)

## @knitr incu-ppb
# creating final partner's previous bout variable
incu_aves<-incu_new2%>%group_by(cycle_tag)%>%summarise(ppb_ave_no_hours=mean(part_prev_bout)) # calculate mean trip duration for each bird in each season
incu_avesdf<- data.frame(incu_aves) # convert this into a data frame
incu_new2<-merge(incu_new2, incu_avesdf, by.x = "cycle_tag", by.y="cycle_tag") # add mean to original data set
incu_new2<-incu_new2%>%mutate(ppb_ave_diff=part_prev_bout-ppb_ave_no_hours) # subtract raw partners previous bout from mean
incu_new2$ppb_ave_diff_sc<-scale(incu_new2$ppb_ave_diff) # scale new variable 
...
## @knitr brood-res
#rescaling
brood_new2$age_sc<-scale(brood_new2$age)
brood_new2$p_age_sc<-scale(brood_new2$p_age)
brood_new2$age_sq_sc<-scale(brood_new2$age_sq)
brood_new2$p_age_sq_sc<-scale(brood_new2$p_age_sq)
brood_new2$pers_sc<-scale(brood_new2$pers)
brood_new2$p_pers_sc<-scale(brood_new2$p_pers)
brood_new2$num_days_sc<-scale(brood_new2$NumDays)


## @knitr brood-pers-cat
# add pers categorical variable brooding 
# FOR GRAPHICAL REPRESENTATION ONLY, NOT TO BE INCLUDED IN ANALYSIS

for (a in 1:(nrow(brood_new2))) {
  if (brood_new2$pers_sc[a] < 0)
  {brood_new2$pers_type[a]<- "shy"}
  else {brood_new2$pers_type[a] <-"bold"}
}

for (a in 1:(nrow(brood_new2))) {
  if (brood_new2$p_pers_sc[a] < 0)
  {brood_new2$p_pers_type[a]<- "shy"}
  else {brood_new2$p_pers_type[a] <-"bold"}
}

brood_new2$pers_type<-as.factor(brood_new2$pers_type)
brood_new2$p_pers_type<-as.factor(brood_new2$p_pers_type)

## @knitr brood-ppb
# creating final partner's previous bout variable
brood_aves<-brood_new2%>%group_by(cycle_tag)%>%summarise(ppb_ave_no_hours=mean(part_prev_bout)) # calculate each individual's mean trip duration
brood_avesdf<- data.frame(brood_aves) # converted into a dataframe
brood_new2<-merge(brood_new2, brood_avesdf, by.x = "cycle_tag", by.y="cycle_tag") # add new mean to original data set
brood_new2<-brood_new2%>%mutate(ppb_ave_diff=part_prev_bout-ppb_ave_no_hours) # subtract previous partner bout from mean
brood_new2$ppb_ave_diff_sc<-scale(brood_new2$ppb_ave_diff) # scale variable 
...
unique(total_df4$cycle) #6 breeding seasons
unique(total_df4$age) #no ages
with(total_df4, tapply(pair_id, cycle, FUN = function(x) length(unique(x)))) # pair sample size per year
with(total_df4, tapply(tag, age, FUN = function(x) length(unique(x))))
total_df4$for_trip<-total_df4$part_prev_bout
...
mean(total_df4$age)
sd(total_df4$age)
total_df4%>%group_by(sex)%>%summarise(average=mean(no_hours),med=median(no_hours), sd=sd(no_hours))
total_df4%>%group_by(partner_type)%>%summarise(average=mean(no_hours),med= median(no_hours), sd=sd(no_hours))
total_df4%>%group_by(breed_stage)%>%summarise(average=mean(no_hours),med=median(no_hours), sd=sd(no_hours))
total_df4%>%group_by(cycle)%>%summarise(average=mean(no_hours),med=median(no_hours), sd=sd(no_hours))
...
## @knitr br-ave
modavm1_brood<-model.avg(nest_m1_brood, delta<2, fit = TRUE) # model averaging
modavm1_brood
